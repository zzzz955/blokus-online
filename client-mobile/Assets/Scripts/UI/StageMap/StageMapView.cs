...
namespace BlokusUnity.UI
{
    ...
    /// </summary>
    public class StageMapView : BaseUIPanel
    {
        [Header("UI 컴포넌트")]
        ...
        {
            if (scrollRect == null || stagesPerRow <= 0) return;

            int rowIndex = (stageNumber - 1) / stagesPerRow;
            int rowCount = Mathf.CeilToInt((float)totalStages / stagesPerRow);

            float normalized = 1f;
            if (rowCount > 1)
            {
                normalized = 1f - (rowIndex / (float)(rowCount - 1));
            }
